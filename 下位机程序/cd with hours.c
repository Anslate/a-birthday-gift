...
void encode(unsigned long Msecs,unsigned int *hours,uchar *mins,uchar *secs){
	//小时
	unsigned int h=Msecs/3600;//总相差小时数
	*hours=h/1000;
	*hours=*hours<<4;
	*hours|=(h%1000)/100;
	*hours=*hours<<4;
	...
	*hours|=h%10;
	//分钟
	unsigned int m=(Msecs%3600)/60;//总相差分钟数
	*mins=m/10;
	*mins=*mins<<4;
	*mins|=m%10;
	//秒
	unsigned int s=Msecs%60;//总相差秒数
	*secs=s/10;
	*secs=*secs<<4;
	*secs|=s%10;
}
...
void main(){
	unsigned int hours;//BCD码，剩余的小时数
	uchar mins;//BCD码，剩余的分钟数
	uchar secs;//BCD码，剩余的秒数
	...
	while(1){//主循环
		...
		encode(Msecs,&hours,&mins,&secs);
		shift(BCD2PINh(hours)|0x00000080);
		P0=BCD[secs&0x0F];
		P1=BCD[(secs>>4)&0x0F];
		P2=BCD[mins&0x0F]|0x80;
		P3=BCD[(mins>>4)&0x0F];
		Delay1s();
		Msecs-=1;
		count+=1;
